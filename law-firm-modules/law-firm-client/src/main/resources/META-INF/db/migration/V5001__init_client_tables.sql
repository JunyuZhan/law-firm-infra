...
CREATE TABLE client_follow_up (
    ...
    tenant_id BIGINT DEFAULT 0 COMMENT '租户ID',
    client_id BIGINT NOT NULL COMMENT '客户ID',
    follow_up_type VARCHAR(50) DEFAULT 'phone' COMMENT '跟进类型(phone-电话跟进,email-邮件跟进,visit-实地拜访,meeting-会议沟通,other-其他)',
    content TEXT COMMENT '跟进内容',
    result VARCHAR(255) COMMENT '跟进结果',
    business_id BIGINT COMMENT '关联业务ID（案件/合同）',
    business_type VARCHAR(50) COMMENT '业务类型',
    next_follow_time DATETIME COMMENT '下次跟进时间',
    is_remind TINYINT DEFAULT 0 COMMENT '是否需要提醒(0-否,1-是)',
    remind_time DATETIME COMMENT '提醒时间',
    remind_type VARCHAR(50) COMMENT '提醒方式（邮件、短信等）',
    assignee_id BIGINT COMMENT '负责人ID',
    status TINYINT DEFAULT 0 COMMENT '跟进状态(0-待跟进,1-已完成,2-已取消)',
    version INT DEFAULT 0 COMMENT '乐观锁版本号',
    sort INT DEFAULT 0 COMMENT '排序',
    deleted TINYINT DEFAULT 0 COMMENT '删除标记(0-正常,1-删除)',
    create_time DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
