#!/bin/bash
echo "正在启动律师事务所管理系统 - 开发环境(无Redis)..."

# 设置环境变量
export SPRING_PROFILES_ACTIVE=dev-noredis
...
if [ "$choice" = "1" ]; then
    echo "使用Maven启动..."
    ./mvnw spring-boot:run -pl law-firm-api -Dspring-boot.run.profiles=dev-noredis
elif [ "$choice" = "2" ]; then
    echo "使用Java运行Jar包..."
